...
class CartPoleTreeNode:
    def __init__(self, state, time_step, strategy, id):
        ...
        self.is_final = False
        self.max_depth = 1
        self.avg_max_depth = [1, 1]
        self.max_unexplored_depth = [1, 1]
        self.avg_risk_weighted_score = self.update_risk_weighted_score()
    ...
    def mark_final(self):
        ...
        self.avg_risk_weighted_score = [0, 0]
    ...
    def update(self):
        if not self.children[0] and not self.children[1]:
            self.max_depth = [self.avg_max_depth(self.children[0]), self.avg_max_depth(self.children(1))]
            self.max_unexplored_depth = [self.avg_max_unexplored_depth(self.children[0]), self.avg_max_unexplored_depth(self.children(1))]
            if self.max_unexplored_depth >= 0.5:
            ...
            i = 0 if not self.children[0] else 1
            self.max_depth = max(self.avg_max_depth(self.children[1]), 1)
            self.max_unexplored_depth = self.avg_max_unexplored_depth(self.children[1]) + 1
            self.holes_beneath = self.average_holes_beneath(self.children[i]) + 1
        self.visited += 1
    ...
    def register_move(self, new_state, new_time_step, direction):
        ...
        if bucket in self.children[direction]:
            self.children[direction][bucket] = self.strategy
        else:
            id = self.id + str(self.most_recent_choice)
            self.children[direction][bucket] = CartPoleTreeNode(new_state, new_time_step, self.strategy, id)
        return self.children[direction][bucket]

    def child_has_unexplored(self, child):
        for state, node in self.children[child]:
            if node.max_unexplored_depths > 0:
                return True
        return False
    ...
    def risk_weighted_score_of_child(self, child):
        ...
        for key, node in child.items:
            ...
            sum += node.visited * node.best_risk_weighted_score()
            odds_of_unvisited += node.odds_of_unvisited_below if node.max_unexplored_depths != 0 else 0
        sum /= visited
        return sum + 1 + RISK_THRESHOLD*odds_of_unvisited

    def best_risk_weighted_score(self):
        return max(self.avg_risk_weighted_score)
    ...
    def average_max_depth(self, child):
        ...
        for key, node in child.items:
            visited += node.visited
            sum += node.visited * max(node.max_depth)
        sum /= visited
        return sum + 1
    ...
    def average_max_unexplored_depth(self, child):
        ...
        for key, node in child.items:
            ...
            more_than_1 += node.visited * (1 if node.max_unexplored_depth >= 1 else 0)
        sum /= visited
    ...
    def average_holes_beneath(self, child):
        ...
        for key, node in child.items:
            visited += node.visited
            sum += node.visited * sum(node.holes_beneath)
        sum /= visited
        return sum
    ...
    def balanced(self):
        left_score = 1 + RISK_THRESHOLD if not self.children[0] else self.avg_risk_weighted_score[0]
        right_score = 1 + RISK_THRESHOLD if not self.children[0] else self.avg_risk_weighted_score[1]
